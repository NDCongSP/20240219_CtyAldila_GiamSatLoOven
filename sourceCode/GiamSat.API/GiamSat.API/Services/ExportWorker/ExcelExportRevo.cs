...
namespace GiamSat.API.Services.ExportWorker
{
    public class ExcelExportRevo
    {
        ...
        public byte[] GenerateExcelFileAsync(
            ...
            string dateQuery,
            RevoReportMode mode,
            RevoShaftScopeKind shaftScope,
            bool useGridWhenAvailable,
            IReadOnlyList<RevoStepRow>? stepRows,
            IReadOnlyList<RevoShaftRow>? shaftRows,
            IReadOnlyList<RevoHourRow>? hourRows,
            List<RevoDropdownModel>? revoList = null)
        {
            ...
            var scopeLabel = shaftScope == RevoShaftScopeKind.Finished
            ...
            if (revoList == null || revoList.Count == 0)
            {
                revoList = data
                    ...
                    .Select(x => new RevoDropdownModel
                    {
                        Id = x.RevoId!.Value,
            ...
            {
                RevoReportMode.ByShaft => 9,
                RevoReportMode.ByHour => 1 + revoList.Count * 2,
                _ => 12
            };
            ...
            var shaftLine = shaftScope == RevoShaftScopeKind.Finished
            ...
            if (useGridWhenAvailable && mode == RevoReportMode.ByStep && stepRows is { Count: > 0 })
                currentRow = WriteStepFromGrid(ws, currentRow, colCount, stepRows);
            else if (useGridWhenAvailable && mode == RevoReportMode.ByShaft && shaftRows is { Count: > 0 })
                currentRow = WriteShaftFromGrid(ws, currentRow, colCount, shaftRows);
            else if (useGridWhenAvailable && mode == RevoReportMode.ByHour && hourRows is { Count: > 0 })
        ...
        private static int WriteStepFromGrid(IXLWorksheet ws, int currentRow, int colCount, IReadOnlyList<RevoStepRow> rows)
        {
        ...
        private static int WriteShaftFromGrid(IXLWorksheet ws, int currentRow, int colCount, IReadOnlyList<RevoShaftRow> rows)
        {
            ws.Cell(currentRow, 1).Value = "STT";
        ...
        private static int WriteHourFromGrid(
            IXLWorksheet ws,
            int currentRow,
            IReadOnlyList<RevoHourRow> rows,
            List<RevoDropdownModel> revoList)
        {
            ...
            List<RevoDropdownModel> revoList)
        {
            ...
            if (shaftScope == RevoShaftScopeKind.Finished)
            {
            ...
            if (mode == RevoReportMode.ByStep)
            {
                ...
                foreach (var x in normalized.OrderBy(x => x.Started))
                {
                    ...
                    var warn = shaftScope == RevoShaftScopeKind.Total
            ...
            if (mode == RevoReportMode.ByShaft)
            {
                ws.Cell(currentRow, 1).Value = "STT";
                ws.Cell(currentRow, 2).Value = "Tên Shaft";
                ws.Cell(currentRow, 3).Value = "Part";
                ...
                foreach (var r in shaftRows)
                {
                    ...
                    var warn = shaftScope == RevoShaftScopeKind.Total && !finishedShafts.Contains(r.ShaftGuid);
                    if (warn)
                        ws.Range(outRow, 1, outRow, colCount).Style.Fill.SetBackgroundColor(WarningFill);
                    outRow++;
                }
        ...
        public static List<RevoHourRow> MapHourRowsFromRaw(
            List<FT09_RevoDatalog> data,
            RevoShaftScopeKind shaftScope,
            List<RevoDropdownModel> revoList)
        {
            ...
            return normalized
                ...
                .Select(g =>
                {
                    ...
                    return new RevoHourRow
                    {
                        ...
                        EndedAt = end,
                        TotalTime = totalTime,
                        HighlightIncomplete = shaftScope == RevoShaftScopeKind.Total && incompleteInHour > 0,

...
namespace GiamSat.API.Services.ExportWorker
{
    public class ExcelExportRevo
    {
        ...
        private static IXLWorksheet CreateNewSheet(
            ...
            int colCount)
        {
            string sheetName = sheetIndex == 1 ? baseName : $"{baseName} ({sheetIndex})";
            ...
            List<RevoDropdownModel>? revoList = null)
        {
            ...
            int colCount = mode switch
            {
                ...
                RevoReportMode.ByHour => 1 + (revoList?.Count ?? 0) * 2,
                _ => 12
            };

            if (mode == RevoReportMode.ByStep && stepRows != null)
            {
                WriteStepFromGrid(wb, "Báo cáo REVO", dateQuery, shaftLine, colCount, stepRows);
            }
            else if (mode == RevoReportMode.ByShaft && shaftRows != null)
            {
                WriteShaftFromGrid(wb, "Báo cáo REVO", dateQuery, shaftLine, colCount, shaftRows);
            }
            else if (mode == RevoReportMode.ByHour && hourRows != null && revoList != null)
            {
        ...
        private static void WriteStepFromGrid(XLWorkbook wb, string baseName, string dateQuery, string shaftLine, int colCount, IReadOnlyList<RevoStepRow> rows)
        {
            int sheetIndex = 1;
        ...
        private static void WriteShaftFromGrid(XLWorkbook wb, string baseName, string dateQuery, string shaftLine, int colCount, IReadOnlyList<RevoShaftRow> rows)
        {
            int sheetIndex = 1;
            var ws = CreateNewSheet(wb, baseName, sheetIndex, dateQuery, shaftLine, colCount);
